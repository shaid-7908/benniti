...
class Views {
    ...
    if ($organizationColumnIndex !== false) {
        ...
        foreach ($optyData as $opty) {
            ...
            $html.='    <td>'. getIfSet($opty, "headline");
            $html.='</td>' . PHP_EOL;
            if (in_array("organization", $columns))
            ...
            if (in_array("view", $actions)) {
                $html.='    <a class="btn btn-info btn-sm" href="opportunityView.php?opportunityid=' . getIfSet($opty, "public_id") . '">View</a>'. PHP_EOL;
            }
            if (in_array("edit", $actions)) {
               $html.='    <a class="btn btn-secondary btn-sm editbutton" href="#public_id=' . getIfSet($opty, "public_id") . '" data-id="'.getIfSet($opty, "public_id").' ">Edit</a>'. PHP_EOL;
            }
            ...
            if (in_array("delete", $actions)) {
                $html.='    <a class="btn btn-danger btn-sm" onclick="return confirm(\'Are you sure you want to delete this opportunity? This action cannot be undone.\')" ';
                $html.='href="?action=delete_opportunity&opportunityid=' . getIfSet($opty, "public_id") . '">Delete</a>';
            }
            $html.='    </td>' . PHP_EOL;
            $html.='    </tr>' . PHP_EOL;
        }
